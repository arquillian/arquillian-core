...
IMPORTANT: Remember to register the SPI by creating a file `META-INF/services/org.jboss.arquillian.container.test.spi.client.deployment.AutomaticDeployment` containing the fully qualified name to your implementation.

Moreover, `@BeforeDeployment` method annotation is created to allow test writer to modify the archive before it is deployed.
