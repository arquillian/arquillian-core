...
public class ContainerStopper
{
   ...
   @Inject
   private Event<BeforeStop> before;
   
   @Inject
   private Event<AfterStop> after;
   
   public void startAllContainers(@Observes AfterSuite event) throws Exception
   {
      for(Container container : registry.get().getContainers())
      {
         ...
         try
         {
            ...
            before.fire(new BeforeStop(deployableContainer));
            
            deployableContainer.stop();
            
            after.fire(new AfterStop(deployableContainer));
         }
         finally
         {
